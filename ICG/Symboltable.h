...
public:
    ...
    int insertSymbol(string name,string type,string data_type,string asm_name){
        SymbolInfo *temp = new SymbolInfo(name,type,data_type,asm_name) ;
        if(head==0){
            temp->setNext(0) ;
            temp->setPrev(0) ;
            head = temp ;
    ...
    int insertSymbol(string name,string type,string data_type,string asm_name,int para){
        SymbolInfo *temp = new SymbolInfo(name,type,data_type,asm_name,para) ;
        if(head==0){
            temp->setNext(0) ;
            temp->setPrev(0) ;
            head = temp ;
    ...
    int insertSymbol(string name,string type,string data_type,int length,string asm_name){
        SymbolInfo *temp = new SymbolInfo(name,type,data_type,length,asm_name) ;
        if(head==0){
            temp->setNext(0) ;
            temp->setPrev(0) ;
            head = temp ;
    ...
    SymbolInfo* getPara(string name){
        int pos = 0 ;
        SymbolInfo *temp = head ;
        while (temp!=0){
            if(temp->getName()==name && temp->getPara()==1){
                return temp ;
            }
            temp = temp->getNext();
            pos++ ;
    ...
    int deleteSymbol(string name){
     ...
     void printList(){
        ...
        while(temp!=0){
            out<<"<"<<temp->getName()<<","<<temp->getType()<<","<<temp->get_data_type()<<","<<temp->getPara()<<"> ";
...
public:
    ...
    SymbolInfo* getPara(string name){
        int index = hash_function(name) ;
        SymbolInfo *temp = list[index].getPara(name);
        return temp ;
    }
    ...
    int insertSymbol(string name,string type,string data_type,string asm_name){
        int index = hash_function(name) ;
        if(list[index].searchSymbol(name)==-1){
            list[index].insertSymbol(name,type,data_type,asm_name) ;
            return index ;
        }
        return -1;
    }
    ...
    int insertSymbol(string name,string type,string data_type,string asm_name,int para){
        int index = hash_function(name) ;
        if(list[index].searchSymbol(name)==-1){
            list[index].insertSymbol(name,type,data_type,asm_name,para) ;
            return index ;
        }
        return -1;
    }
    ...
    int insertSymbol(string name,string type,string data_type,int length,string asm_name){
        int index = hash_function(name) ;
        if(list[index].searchSymbol(name)==-1){
            list[index].insertSymbol(name,type,data_type,length,asm_name) ;
            return index ;
        }
        return -1;
    }
...
public:
    ...
    bool insertSymbol(string name,string type,string data_type,string asm_name){
        int pos = tables->insertSymbol(name,type,data_type,asm_name) ;
        if(pos!=-1){
            out<<" Inserted in ScopeTable#"<<tables->getScopeNum()<<" at position "<<pos<<" , "<<llpos<<endl ;
            return true ;
        }
    ...
    bool insertSymbol(string name,string type,string data_type,string asm_name,int para){
        int pos = tables->insertSymbol(name,type,data_type,asm_name,para) ;
        if(pos!=-1){
            out<<" Inserted in ScopeTable#"<<tables->getScopeNum()<<" at position "<<pos<<" , "<<llpos<<endl ;
            return true ;
        }
    ...
    bool insertSymbol(string name,string type,string data_type,int length,string asm_name){
        int pos = tables->insertSymbol(name,type,data_type,length,asm_name) ;
        if(pos!=-1){
            out<<" Inserted in ScopeTable#"<<tables->getScopeNum()<<" at position "<<pos<<" , "<<llpos<<endl ;
            return true ;
        }
    ...
    SymbolInfo* getPara(string name){
        ScopeTable *temp = tables ;
        int i = 0;
        SymbolInfo *symbol = tables->getPara(name) ;
        if(symbol!=0){
            return symbol ;
        }
        else{
            while(temp!=0){
                symbol = temp->getPara(name) ;
                if(symbol!=0){
                    return symbol ;
                }
                temp = temp->getParent();
